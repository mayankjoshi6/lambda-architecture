...
object KafkaStrucutredStreamingJob {

  val kafkaParams = Map[String, Object](
    "bootstrap.servers" -> "localhost:9092",
    "key.deserializer" -> classOf[StringDeserializer],
    "value.deserializer" -> classOf[StringDeserializer],
    "group.id" -> "spark-stream-1",
    "auto.offset.reset" -> "latest",
    "enable.auto.commit" -> (false: java.lang.Boolean)
  )

  def main(args:Array[String]) = {
    val spark=getSparkSession("lambda-spark-strucutred-streaming")
    ...
    val kafkaStream=spark.readStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092")
      .option("subscribe", webLogTopic)
      .load().map(_.getAs[Activity]("value")

    /*kafkaStream.flatMap{row=>
      val record =l.split("\\t")
